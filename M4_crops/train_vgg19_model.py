"""Trains the trainable layers of the vgg19 model.
...
flags.DEFINE_string('tfrecords_dir', cfg.directory['tfrecords'], 'Data directory for storing tfRecords')
...
flags.DEFINE_string('model_dir', 'vgg-e%d-d%d/' % (cfg.hyperparameters['num_epochs'], cfg.dataset['size']), 'Directory for storing model and results')
...
def inputs(train, batch_size, num_epochs):
  ...
  filename = os.path.join(FLAGS.tfrecords_dir,
...
def run_training():
    ...
    with treedom_graph.as_default():
        ...
        network = model.Vgg19("vgg19.npy", trainable=True)
        ...
        folder = FLAGS.train_dir + FLAGS.model_dir + FLAGS.tensorboard_dir
    ...
    with tf.Session(graph=treedom_graph) as sess:
        ...
        print("Finished writing graph")
        
        # Save image
        fig=plt.figure()
        a1=fig.add_subplot(111)
        a1.plot(losses, label="losses")
        fig.savefig(FLAGS.train_dir + FLAGS.model_dir + 'losses.png')
...
shutil.copy2('/home/nagy729krisztina/M4_crops/config.py', directory)

f = open(directory + '/log', 'a+')
f.write('\n\n----CNN CLASSIFICATION based on VGG net TRAINING----\n\n')
f.write('Batch size: %d\n' % (FLAGS.batch_size))
f.write('Number of epochs: %d\n' % (FLAGS.num_epochs))
f.write('Image size: %d * %d * %d = %d\n\n' % (cfg.image['height'], cfg.image['width'], cfg.image['channels'], FLAGS.image_pixels))
