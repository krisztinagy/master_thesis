...
class Vgg19:
    ...
    def build(self, rgb, train_mode=None):
        ...
        self.fc8 = self.fc_layer(self.relu7, 4096, 1, "fc8_modified", trainable=True)
		# name cannot be "fc8", because the program would try to load the weights to 1000 classes.
		# output nodes number modified: 1000 -> 1
	...
	######################################################
    def get_conv_var(self, filter_size, in_channels, out_channels, name, tainable):
        initial_value = tf.truncated_normal([filter_size, filter_size, in_channels, out_channels], 0.0, 0.001)
        filters = self.get_var(initial_value, name, 0, name + "_filters", trainable)

        initial_value = tf.truncated_normal([out_channels], .0, .001)
        biases = self.get_var(initial_value, name, 1, name + "_biases", tainable)
